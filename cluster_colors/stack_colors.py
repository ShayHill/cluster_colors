...
def stack_colors(colors: Pixels) -> StackedColors:
    ...
    :param colors: array of colors, with shape (..., 3) or (..., 4)
    :return: array of stacked colors, with shape (n, 4)
    :raises ValueError: if colors do not have shape (..., 3) or (..., 4)

    Assumes four-channel colors already have a weight in the fourth channel.
    Assumes three-channel vectors are opaque, 8-bit colors and adds a weight of 255.
    """
    match colors.shape[-1]:
        case 4:
            stacked = stack_vectors(colors)
        case 3:
            stacked = stack_vectors(colors, 255)
        case _:
            raise ValueError(
                f"colors must have shape (..., 3) or (..., 4), not {colors.shape}"
            )
    return stacked
